...
def draw_squares_from_circle(n, circle, window):
    ...
    for k in range(n):
        ...
        c1 = rg.Point(c1.x + circle.radius, c1.y + circle.radius)
        c2 = rg.Point(c2.x + circle.radius, c2.y + circle.radius)
    window.render()
    """
...
def run_test_draw_circles_from_rectangle():
    ...
    title = 'Test 3 of DRAW_SQUARES_FROM_RECTANGLE:'
    ...
    rectangle2.outline_thickness = 7
    draw_circles_from_rectangle(10, 6, rectangle2, window2)
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    for k in range(n):
        circle = rg.Circle(
            rg.Point(rectangle.get_center().x, rectangle.get_upper_left_corner().y - width * (1 / 2 + k)), width / 2)
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    total = 0
    for k in range(n):
        x = rg.Point(center1.x - k / 2 * width1, center1.y + k / 2 * height)
        y = rg.Point(center2.x - k / 2 * width2, center2.y + k / 2 * height)
        line = rg.Line(x, y)
        line.thickness = 5
        if total % 2 == 0:
